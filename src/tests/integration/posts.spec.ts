...
describe("Posts", () => {
  ...
  beforeEach(async () => {
    ...
    const userModel = new User({
      ...
      name: "Bob",
      givenName: "Bob",
      email: "bob@yoursanonymously.space",
    ...
    const modUserModel = new User({
      ...
      name: "Mod",
      givenName: "Mod",
      email: "mod@yoursanonymously.space",
      profilePicture: "https://i.imgur.com/2j1RdhZ.png",
      moderator: true,
      verifiedNMIMS: true,
    });
  ...
  describe("GET /posts", () => {
    it("should return a sorted array of posts if no query offered", async () => {
      ...
      expect(res.body[1].verifiedNMIMS).toBe(true);
      ...
      expect(res.body[0].verifiedNMIMS).toBe(true);
    });
  ...
  describe("POST /posts", () => {
    ...
    it("should return 200 if logged in and content is valid", async () => {
      ...
      expect(post?.verifiedNMIMS).toBe(false);
    });
    ...
    it("should return 200 if verified logged in and content is valid", async () => {
      ...
      expect(post?.needsReview).toBe(true);
      expect(post?.comments).toHaveLength(0);
      expect(post?.verifiedNMIMS).toBe(true);
    });
  });
